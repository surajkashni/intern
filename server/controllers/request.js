...
exports.helpreq=async(req,res)=>{
    ...
    exports.donatersForReq=async(req,res)=>{
        let d= await User.find({$and:[{role:"donor"},{verified:"true"},{status:"active"},{category:req.body.category}]})
        .populate("infoDon")
        .sort([["createdAt","desc"]])
        .exec();
        res.json(d);
    ...
    exports.requestersForDon=async(req,res)=>{
        let d= await User.find({$and:[{role:"requester"},{verified:"true"},{status:"active"},{category:req.body.category}]})
        .populate("infoReq")
        .sort([["createdAt","desc"]])
        .exec();
        res.json(d);
